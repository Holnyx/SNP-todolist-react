...
const CustomCheckboxAllTasks: FC<CustomCheckboxAllTasksItems> = ({ tasks, onClickHandler }) => {
    ...
    const changeStyleCheckbox = cx({
        ...
        [s['img--show']]: tasks.some(el => !el.isDone),
        [s['img--change-color']]: tasks.every(el => el.isDone)
    });

    return (
        <>
            <input className={cx(s.checkbox)} type="checkbox" id="checkbox-checked-all" />
            <label htmlFor="checkbox-checked-all" className={s['custom-checkbox']}>
                <svg onClick={onClickHandler} id="img" className={changeStyleCheckbox} width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default CustomCheckboxAllTasks
